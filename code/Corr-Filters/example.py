...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        ...
        k = 0
        g = np.linalg.inv(((k*np.diag(np.ones(X.shape[0])))+X@X.T))@X@Y
        g= g.reshape(dsize)
        response = scipy.ndimage.correlate(img,g)
        plt.imsave('lambda_0.jpg',response)
        
        conv_response = scipy.ndimage.convolve(img,g)
        plt.imsave('lambda_0_conv_no_flip.jpg',conv_response)
        
        conv_response = scipy.ndimage.convolve(img,np.flip(g))
        plt.imsave('lambda_0_conv.jpg',conv_response)

        k = 1
        g = np.linalg.inv(((k*np.diag(np.ones(X.shape[0])))+X@X.T))@X@Y
        g= g.reshape(dsize)
        response = scipy.ndimage.correlate(img,g)
        plt.imsave('lambda_1.jpg',response)
        
        conv_response = scipy.ndimage.convolve(img,g)
        plt.imsave('lambda_1_conv_no_flip.jpg',conv_response)

        conv_response = scipy.ndimage.convolve(img,np.flip(g))
        plt.imsave('lambda_1_conv.jpg',conv_response)
